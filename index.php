...
if(preg_match('/^\/([Ss][Tt][Aa][Rr][Tt])(.*)/',$text)){
	...
	if($match[2]!=null){
		...
		$my_subset=json_decode(file_get_contents('data/members/'.$chat_id.'/my_subset.json'),true);
		if($match[2]!=$chat_id && $match[2]!=$admin && in_array($match[2],$admins)==false && in_array($chat_id,$my_subset)==false){
			...
			$my_subset[]=$chat_id;
			saveJson('data/members/'.$chat_id.'/my_subset.json',$my_subset);
...
if($text=='ساخت ربات🛠' && $step[$chat_id]=='home'){
	if($robot_list!=null){
		if(in_array($chat_id,$vips)==false && $my_robots>=$settings['free_max_robot'] && $chat_id!=$admin && in_array($chat_id,$admins)==false){
...
if($step[$chat_id]=='send_all' && $text!=$s){
	if(isset($text)){
		...
		foreach($members as $key){
			sendMessage($key,$text,$parse_mode,null);
		}
...
if($text==$s && $step[$chat_id]=='managemant'){
	if(count($members)>1){
		endMessage($chat_id,'♦️لطفا پیام خود را ارسال کنید :','HTML',$button_back_managemant);
